...
export const metadata: Metadata = { title: 'Waarborgen' };
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-ink-900">Waarborgen</h1>
          <p className="mt-1 text-sm text-ink-500">{guarantees.length} waarborgdossier(s).</p>
        </div>
      ...
      {guarantees.length === 0 ? (
        <EmptyState title="Geen waarborgdossiers" />
      ) : (
          ...
          {guarantees.map((guarantee: any) => (
            <div key={guarantee.id} className="rounded-2xl border border-ink-100 bg-white p-4 sm:p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-sm text-ink-500">
                    Reservering: {guarantee.reservations?.reference ?? '—'}
                  </p>
                </div>
              ...
              <div className="mt-3 grid grid-cols-1 gap-2 text-sm text-ink-600 sm:grid-cols-2 lg:grid-cols-4">
                <p><span className="text-ink-400">Klant : </span>{guarantee.clients?.first_name} {guarantee.clients?.last_name}</p>
                <p className="break-all"><span className="text-ink-400">E-mail: </span>{guarantee.clients?.email}</p>
                <p><span className="text-ink-400">Bedrag : </span>{formatPrice(guarantee.amount)}</p>
                {guarantee.declared_transfer_date && (
                  <p><span className="text-ink-400">Overschrijving aangegeven op: </span>{formatDate(guarantee.declared_transfer_date)}</p>
                )}
              </div>

              {guarantee.declared_bank_name && (
                <div className="mt-2 rounded-lg bg-sand-100/60 px-3 py-2 text-xs text-ink-500">
                  Uitgevende bank: {guarantee.declared_bank_name}
                  {guarantee.declared_reference && <> · Ref.: {guarantee.declared_reference}</>}
              ...
              <div className="mt-3 flex flex-col gap-3 border-t border-ink-100 pt-3 sm:flex-row sm:items-center sm:justify-between">
                <span className="text-xs text-ink-400">Aangemaakt op {formatDateTime(guarantee.created_at)}</span>
                <StatusSelect
