...
import { ArrowLeft, CalendarDays, CreditCard, ArrowRight } from 'lucide-react';
import { getPropertyBySlug } from '@/lib/data/properties';

export const metadata: Metadata = { title: 'Reageren op deze advertentie' };
  ...
  return (
    <div className="container-app py-8 sm:py-16">
      <Link href={`/appartements/${property.slug}`} className="inline-flex items-center gap-1.5 text-sm font-medium text-ink-500 hover:text-ink-900">
        <ArrowLeft className="h-4 w-4" /> Terug naar de woning
      </Link>
      <div className="mx-auto mt-8 max-w-4xl text-center">
        ...
        <h1 className="mt-2 text-display-sm font-extrabold text-ink-900 sm:text-display-md">
          Wilt u de reserveringskosten betalen of een bezichtiging aanvragen?
        </h1>
        <div className="mt-9 grid gap-5 sm:grid-cols-2">
          <Choice href={`/appartements/${property.slug}/reserver`} icon={CreditCard} title="Reserveringskosten betalen" description="Vul uw dossier in om de reserveringskosten te betalen." />
          <Choice href={`/appartements/${property.slug}/visite`} icon={CalendarDays} title="Bezichtiging" description="Vraag een datum aan om deze woning te bezichtigen." />
        </div>
      </div>
    </div>
  );
...
function Choice({ href, icon: Icon, title, description }: { href: string; icon: typeof CalendarDays; title: string; description: string }) {
  return (
    <Link href={href} className="group rounded-2xl border border-ink-100 bg-white p-7 text-left shadow-soft transition hover:-translate-y-1 hover:border-canal-300 hover:shadow-lifted">
      ...
      <span className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-canal-700">Doorgaan <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span>
    </Link>
  );
}
